...
def update_db():
    ...
    for repo, url in repo_urls.items():
        try:
            # Prepare the payload for each repository
            payload = {
                "repo_url": url,
                "tools": ["Gitleaks", "Guarddog", "Safety", "Bearer"]
            }

            # Send the request to the API Gateway to analyze the repository
            ...
            if response.status_code == 200:
                ...
                # Log error for this repository if the status code is not 200
                results.append({
                    "repo": repo,
                    "status": "error",
    ...
    # Return a structured response summarizing the results
    return jsonify({"message": "Database update completed", "results": results}), 200
